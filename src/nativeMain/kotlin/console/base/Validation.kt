...
enum class Validation {
    ...
    , TIMEZONE_VALIDATION { override fun isValid(entry : Any) = ( (entry is Int) && (entry >= -12) && (entry <= 14) ) }
    ...
    companion object {
        ...
        fun isValidDateStringFormat(dateString : String, report : Boolean = true) : Boolean {
            ...
            val isValid = dateString.contains(Regex("^\\d{4}${EntryState.DATE_ENTRY.getDelim()}\\d{2}${EntryState.DATE_ENTRY.getDelim()}\\d{2}$") )
        ...
        fun isValidDate(year : Int, mon : Int, mday : Int, report : Boolean = true) : Boolean {
            ...
            val dateString = "$year${EntryState.DATE_ENTRY.getDelim()}$mon${EntryState.DATE_ENTRY.getDelim()}$mday"
        ...
        fun isValidTimeStringFormat(timeString : String, report : Boolean = true) : Boolean {
            ...
            val isValid = timeString.contains(Regex("^\\d{2}${EntryState.TIME_ENTRY.getDelim()}\\d{2}${EntryState.TIME_ENTRY.getDelim()}\\d{2}$") )
        ...
        fun isValidTime(hour : Int, min : Int, sec : Int, report : Boolean = true) : Boolean {
            ...
            val timeString = "$hour${EntryState.TIME_ENTRY.getDelim()}$min${EntryState.TIME_ENTRY.getDelim()}$sec"

            return (RenderValidations.getRenderIsValidTimeString(timeString, isValid, report) == RenderValidations.isValidString)
        }

        fun isValidDoubleFormat (doubleString : String, report : Boolean = true) : Boolean {

            val isValid = doubleString.contains(Regex("^(-?)([0-9]+).([0-9]+)$") )

            return (RenderValidations.getRenderIsValidAnyString(doubleString, isValid, report) == RenderValidations.isValidString)
        }

        fun isValidIntFormat (doubleString : String, report : Boolean = true) : Boolean {

            val isValid = doubleString.contains(Regex("^(-?)([0-9]+)$") )
