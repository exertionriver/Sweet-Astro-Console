...
enum class RenderSign {
    ...
    companion object {
        ...
        fun getSynLabel(sign : Sign) : String = Constants.KMAG + RenderSign.fromName(sign.toString())!!.getLabel() + Constants.KNRM

        fun getElementLabel(sign : Sign) : String = when {
            (sign.getElement() == SignElement.FIRE_ELEMENT) -> Constants.KRED + RenderSign.fromName(sign.toString())!!.getLabel() + Constants.KNRM
            (sign.getElement() == SignElement.EARTH_ELEMENT) -> Constants.KYEL + RenderSign.fromName(sign.toString())!!.getLabel()+ Constants.KNRM
            (sign.getElement() == SignElement.AIR_ELEMENT) -> Constants.KGRN + RenderSign.fromName(sign.toString())!!.getLabel() + Constants.KNRM
            (sign.getElement() == SignElement.WATER_ELEMENT) -> Constants.KBLU + RenderSign.fromName(sign.toString())!!.getLabel() + Constants.KNRM
            else -> RenderSign.fromName(sign.toString())!!.getLabel()
        }
